...
export default function ProjectTableEditor() {
  ...
  const rows = useMemo(() => {
    if (!progress) return [];
    const arr = Object.entries(progress).map(([id, record]) => ({ id, ...record }))
   console.log("rows = useMemo()：", arr)
    return arr;
  }, [progress]);
    ...
    // Standalone boolean fields
    {
      ...
      Cell: ({ row }) => (
        <EditableCheckbox 
          ...
          onChange={(e) =>{
            console.time('checkbox-react');
            handleChange(row.original.id, 'arol', null, e.target.checked)
            console.timeEnd('checkbox-react');
        }}
        />
      ),
    },
